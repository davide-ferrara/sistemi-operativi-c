...
int main(void){
    ...
    int T1 = pthread_create(&T1_ID, NULL, row_checker, valid_sudoku);
    int T2 = pthread_create(&T1_ID, NULL, col_checker, valid_sudoku);
    
    if(T1 || T2 != 0) return -1;

    if(pthread_join(T1, NULL) || pthread_join(T2, NULL) != 0) return -1;
